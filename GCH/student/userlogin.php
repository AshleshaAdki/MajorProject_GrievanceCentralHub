...
if (isset($_POST['submit'])) {
    ...
    if ($num > 0) {
        ...
        $status = 1;
        $log = mysqli_query($db, "insert into userlog(uid,username,userip,status) values('" . $_SESSION['id'] . "','" . $_SESSION['login'] . "','" . $_SERVER['REMOTE_ADDR'] . "','$status')");
        header("location:index.php");
        exit();
    } else {
        $_SESSION['login'] = $_POST['email']; // Corrected from 'username' to 'email'
        $status = 0;
        mysqli_query($db, "insert into userlog(username,userip,status) values('" . $_SESSION['login'] . "','" . $_SERVER['REMOTE_ADDR'] . "','$status')");
		...
		<section class="form">
			...
			<form class="student-form" method="post">
				...
				<fieldset>
					...
					<label for="username">User Id <i class="fa fa-user-circle" aria-hidden="true"></i><br><input type="text" name="email" id="fullname-user" placeholder="Enter Email Id" required value="" autofocus></label><br>

					<label for="password-registration-user">Password <i class="fa fa-key" aria-hidden="true"></i><br><input type="password" name="password" id="txtPassword" value="" placeholder="Enter your Password here"></label><br>
